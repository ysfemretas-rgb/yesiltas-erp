  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">{new Date().toLocaleDateString('tr-TR')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2"><Clock size={20} className="text-blue-500"/>Son Servis Kayıtları</h2>
            <Link href="/dashboard/services" className="text-sm text-green-600 hover:text-green-700 flex items-center gap-1">Tümü <ChevronRight size={16} /></Link>
          </div>
            ...
            {recentServices.length === 0 ? <p className="text-sm text-gray-500 text-center py-4">Henüz servis kaydı yok</p> : (
              recentServices.map((service) => (
                <div key={service.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                  <div>
                    <p className="font-medium text-sm text-gray-900 dark:text-white">{service.customers?.full_name || 'İsimsiz'}</p>
                    <p className="text-xs text-gray-500">{service.device_type} - {service.problem}</p>
                  </div>
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    service.status === 'Bekliyor' ? 'bg-yellow-100 text-yellow-700' :
                    service.status === 'İşlemde' ? 'bg-blue-100 text-blue-700' :
                    service.status === 'Tamamlandı' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'
                  }`}>{service.status}</span>
                </div>
              ))
            )}
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2"><AlertTriangle size={20} className="text-red-500"/>Düşük Stok Uyarıları</h2>
            <Link href="/dashboard/inventory" className="text-sm text-green-600 hover:text-green-700 flex items-center gap-1">Tümü <ChevronRight size={16} /></Link>
          </div>
          <div className="space-y-3">
