...
export default function DashboardPage() {
  const [stats, setStats] = useState({ devicesWaiting: 0, devicesReady: 0, todaySales: 0, lowStock: 0 });
  const [recentDevices, setRecentDevices] = useState<any[]>([]);

  useEffect(() => { fetchStats(); fetchRecentDevices(); }, []);
  ...
  const fetchStats = async () => {
    ...
    setStats({ devicesWaiting: waiting || 0, devicesReady: ready || 0, todaySales: todaySales, lowStock: low || 0 });
  };

  const fetchRecentDevices = async () => {
    const { data } = await supabase.from("devices").select("*, customers(name)").order("created_at", { ascending: false }).limit(5);
  ...
  return (
    <div>
          ...
          <Link key={card.href + card.label} href={card.href}>
            <div className="card-hover">
              <div className="flex items-center gap-4">
                ...
                <div><p className="text-sm text-slate-400">{card.label}</p><p className={`text-2xl font-bold ${card.color}`}>{card.value}</p></div>
              </div>
            </div>
          </Link>
        ))}
      ...
      <div className="card">
        ...
        {recentDevices.length === 0 ? <p className="text-slate-500 text-center py-8">Henüz cihaz kaydi yok.</p> : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="border-b border-slate-700"><th className="table-header">Musteri</th><th className="table-header">Cihaz</th><th className="table-header">Sikayet</th><th className="table-header">Durum</th><th className="table-header">Tarih</th></tr></thead>
              <tbody>{recentDevices.map((d) => (<tr key={d.id} className="hover:bg-slate-700/30"><td className="table-cell font-medium">{d.customers?.name || "-"}</td><td className="table-cell">{d.brand} {d.model}</td><td className="table-cell max-w-xs truncate">{d.complaint}</td><td className="table-cell"><span className={`badge border ${statusColors[d.status] || "bg-slate-700 text-slate-300"}`}>{d.status}</span></td><td className="table-cell text-slate-500">{new Date(d.created_at).toLocaleDateString("tr-TR")}</td></tr>))}</tbody>
            </table>
          </div>
        )}
      </div>
