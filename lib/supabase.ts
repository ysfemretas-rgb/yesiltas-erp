...
export type Service = { id: string; service_no: string; customer_id: string; device_type: string; device_model: string; imei: string; problem: string; diagnosis: string; status: string; estimated_cost: number; final_cost: number; deposit: number; warranty_months: number; technician: string; notes: string; completed_at: string; created_at: string }
export type Sale = { id: string; sale_no: string; customer_id: string; total_amount: number; discount: number; payment_type: string; paid_amount: number; status: string; created_at: string }
